...
if ($o_check_type eq "juniper") {
	...
	if (defined($resultat_c)) {	
		foreach my $key ( keys %$resultat_c) {
			...
			if ($key =~ /$juniper_operating_state/) {
				...
				#tu.doan create result_t for printing even in case status ok
				$index = $key;
				$index =~ s/^$juniper_operating_state.//;
				@temp_oid=($juniper_operating_descr.".".$index);
				$result_t = $session->get_request( Varbindlist => \@temp_oid);
				if ($$result_t{$juniper_operating_descr.".".$index} eq "noSuchObject") {
					#$print $key,"\t:\t This OID can be walk but cannot be get, Juniper is stupid,ignoring this OID\n"
					$detailed_status .= $key."\t:\t This OID can be walk but cannot be get, Juniper is stupid,IGNORING  OID\n"
				}
				else {
					$num_cards++;
					$tmp_status=$juniper_operating_status[$$resultat_c{$key}];
					#print $key,":",$tmp_status,"\n"; #tu.doan: check OID + status code
					...
					if ($tmp_status == 0) {
						...
						if ($$result_t{$juniper_operating_descr.".".$index} =~ /PCMCIA|USB|Flash|Fan Tray [0-9]$/ && $tmp_status == 3) {
							...
							print $$result_t{$juniper_operating_descr.".".$index};
					...
					#print $$result_t{$juniper_operating_descr.".".$index},"\t:\t", $juniper_operating_status_text[$$resultat_c{$key}],"\n"; #tu.doan: check OID + status code
					#print $key,"\t:\t", $juniper_operating_status_text[$$resultat_c{$key}],"\n"; #tu.doan: check OID + status code
					#$detailed_status .=$$result_t{$juniper_operating_descr.".".$index}."\t:\t". $juniper_operating_status_text[$$resultat_c{$key}]."\n"; #tu.doan: store OID + status code
					$detailed_status .=sprintf("%-20s: %4s\n",$$result_t{$juniper_operating_descr.".".$index},$juniper_operating_status_text[$$resultat_c{$key}] )
				}
			}
		}
		if ($card_output ne "") {$card_output.=", ";}
	...
	if ($final_status == 3) {
		print " === Overall UNKNOWN: ",$output,"\n";
		print "======Detailed status is=====\n".$detailed_status;
		exit $ERRORS{"UNKNOWN"};
	}
	
	if ($final_status == 2) {
		print " === Overall CRITICAL: ",$output,"\n";
		print "======Detailed status is=====\n".$detailed_status;
		exit $ERRORS{"CRITICAL"};
	}

	if ($final_status == 1) {
		print " === Overall WARNING: ",$output,"\n";
		print "======Detailed status is=====\n".$detailed_status;
		exit $ERRORS{"WARNING"};
	}
	
	print " === Overall OK: ",$output,"\n";
	print "======Detailed status is=====\n".$detailed_status;
	exit $ERRORS{"OK"};
}
